...
export function ProfessionalDashboard() {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: kpi.digitalAdoption >= benchmarks.digitalAdoptionTarget ? 'text-[#10B981]' : 'text-[#0EA5E9]',
      bgColor: kpi.digitalAdoption >= benchmarks.digitalAdoptionTarget ? 'bg-[#ECFDF5]' : 'bg-[#F0F9FF]',
    },
  ];

  // Prepare initial data structures
  const concessionByCategory = concessionAnalysis.concessionTypeWise || [];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="border border-[#E0F2FE] bg-[#F8FAFC] shadow-sm p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-br from-[#F0F9FF] to-[#ECFDF5] rounded-lg p-4 border border-[#E0F2FE]">
                    <p className="text-xs text-[#64748B] font-bold uppercase mb-2">💰 Total Outstanding</p>
                    <p className="text-3xl font-black text-[#0F172A]">
                      {formatCurrency(kpi.totalBalance, true)}
                    </p>
                    <p className="text-xs text-[#64748B] mt-2 font-semibold">
                      {formatPercentage((kpi.totalBalance / kpi.totalExpected) * 100)} of expected revenue
                    </p>
                  </div>
                  
                  <div className="bg-gradient-to-br from-[#FEF3C7] to-[#FEF9C3] rounded-lg p-4 border border-[#FDE68A]">
                    <p className="text-xs text-[#78350F] font-bold uppercase mb-2">📊 Total Concession Impact</p>
                    <p className="text-3xl font-black text-[#92400E]">
                      {formatCurrency(kpi.totalConcession, true)}
                    </p>
                    <p className="text-xs text-[#78350F] mt-2 font-semibold">
                      {formatPercentage((kpi.totalConcession / kpi.totalExpected) * 100)} of expected revenue
                    </p>
                  </div>
                </div>
              </Card>
