...
// ========================================
// Data Types from New Excel Files
// ========================================

// Record structure for 2023-24 (Detailed breakdown)
export interface Collection2023Record {
  receiptDate: number;
  receiptNo: number;
  admNo: number;
  studentName: string;
  ...
  excessAmount: number;
  tuitionFees: number;
  gamesSportsActivites: number;
  developmentFund: number;
  computerFees: number;
  annualFees: number;
  labFees: number;
  admissionFee: number;
  smartClassFees: number;
  excessAmt: number;
  totalPaid: number;
  // Concession details
  concessionTuitionFees: number;
  concessionGamesSportsActivites: number;
  concessionDevelopmentFund: number;
  concessionComputerFees: number;
  concessionAnnualFees: number;
  concessionAdmissionFee: number;
  concessionSmartClassFees: number;
  totalConcession: number;
  // Defaulter details
  defaulterTuitionFees: number;
  defaulterGamesSportsActivites: number;
  defaulterDevelopmentFund: number;
  defaulterComputerFees: number;
  defaulterAnnualFees: number;
  defaulterLabFees: number;
  defaulterSmartClassFees: number;
  defaulterTotal: number;
}

// Record structure for 2024-25 and 2025-26 (Simplified)
export interface Collection2024_2025Record {
  receiptDate: number;
  receiptNo: number;
  admNo: number;
  studentName: string;
  ...
  chequeBounceAmount: number;
  lateFee: number;
  readmissionFee: number;
  busFee: number; // Only in 2025-26
  excessAmount: number;
  schoolFees: number;
  admissionFee: number;
  totalPaid: number;
  // Concession details
  concessionSchoolFees: number;
  concessionAdmissionFee: number; // Only in 2025-26
  totalConcession: number;
  // Defaulter details
  defaulterBusFee: number; // Only in 2025-26
  defaulterFees: number;
  defaulterAdmissionFee: number;
  defaulterTotal: number;
}

// Student-wise summary (aggregated data)
...
class DataLoader {
  private collection2023Data: any[] = [];
  private collection2024Data: any[] = [];
  private collection2025Data: any[] = [];
  ...
  async loadData(): Promise<void> {
    ...
    try {
      ...
      if (fs.existsSync(path2023)) {
        ...
        this.collection2023Data = XLSX.utils.sheet_to_json(sheet2023, { defval: 0 });
        console.log(`Loaded ${this.collection2023Data.length} records from 2023-24`);
      }

      // Load 2024-25 Collection
      ...
      if (fs.existsSync(path2024)) {
        ...
        this.collection2024Data = XLSX.utils.sheet_to_json(sheet2024, { defval: 0 });
        console.log(`Loaded ${this.collection2024Data.length} records from 2024-25`);
      }

      // Load 2025-26 Collection
      ...
      if (fs.existsSync(path2025)) {
        ...
        this.collection2025Data = XLSX.utils.sheet_to_json(sheet2025, { defval: 0 });
  ...
  getCollection2023Data(): any[] {
    return this.collection2023Data;
  }

  getCollection2024Data(): any[] {
    return this.collection2024Data;
  }

  getCollection2025Data(): any[] {
  ...
  getDefaulterAnalysis() {
    ...
    uniqueStudents.forEach((record) => {
      ...
      const existing = occupationMap.get(occ) || { total: 0, defaulters: 0, balance: 0 };
      existing.total++;
